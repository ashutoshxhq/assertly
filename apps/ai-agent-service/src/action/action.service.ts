...
function cleanAndTrimContent(
    ...
    const usefulAttributes = ['id', 'class', 'name', 'href', 'src', 'alt', 'title', 'role', 'type', 'value', 'placeholder'];

    function isUsefulAttribute(key) {
        return usefulAttributes.includes(key) || key.startsWith('data-') || key.startsWith('aria-');
    }
        ...
        if (element.type === 'element') {
            ...
            if (element.attributes && element.attributes.length > 0) {
                element.attributes = element.attributes
                    .filter(attr => isUsefulAttribute(attr.key) && attr.value.trim() !== '')
                    .map(attr => ({
                        key: attr.key,
                        value: typeof attr.value === 'string'
                            ? attr.value.trim().length > maxAttributeLength
                                ? attr.value.trim().slice(0, maxAttributeLength) + '...'
                                : attr.value.trim()
                            : attr.value
                    }));
            }
            ...
            element.content = trimmedContent.length > maxContentLength
...
export class ActionService {
    ...
    async findLocator(data: FindLocatorDTO) {
        ...
        const elements = body.querySelectorAll('style, script, img, svg, iframe');
