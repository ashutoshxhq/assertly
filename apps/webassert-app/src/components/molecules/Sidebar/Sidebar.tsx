    ...
    <div className="fixed inset-y-0 left-0 w-[70px] h-[calc(100vh)] overflow-y-hidden z-50" data-tauri-drag-region>
        <nav className="flex h-full min-h-0 flex-col" data-tauri-drag-region>
            <div className="flex flex-col p-4 mt-4">
                <span className="absolute left-1/2 top-1/2 size-[max(100%,2.75rem)] -tranzinc-x-1/2 -tranzinc-y-1/2 [@media(pointer:fine)]:hidden" aria-hidden="true"></span><span data-slot="avatar" className="inline-grid shrink-0 align-middle [--avatar-radius:20%] [--ring-opacity:20%] *:col-start-1 *:row-start-1 outline outline-1 -outline-offset-1 outline-black/[--ring-opacity] dark:outline-white/[--ring-opacity] rounded-full *:rounded-full"><img className="size-full" src="/catalyst.svg" alt="" /></span>
            </div>
            ...
            <div className="flex flex-col gap-2 p-4 border-t border-zinc-950/5  dark:border-white/5 [&amp;>[data-slot=section]+[data-slot=section]]:mt-2.5" data-tauri-drag-region>
