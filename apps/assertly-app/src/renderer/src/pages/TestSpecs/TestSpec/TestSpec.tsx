import { RiArrowLeftSLine, RiEqualizer2Line, RiSave2Line } from 'react-icons/ri'
...
import Loader from '@renderer/components/molecules/Loader'
import { testSpecStepsAtom } from '@renderer/store/test-specs/steps'
...
const TestSpec = () => {
  ...
  const [{ mutate, status: updateStatus }] = useAtom(updateTestSpecMutationAtom)
      ...
      <Tabs defaultValue="step-builder" className="flex flex-col">
        <div className="flex justify-between items-center px-2">
          <div className="flex w-[33.33%] gap-2">
            <div className="px-4 py-2 w-[60px]"></div>
            <Button variant="secondary" onClick={() => navigate(-1)} className="mx-0 px-0 pr-4 pl-2 text-sm">
              {' '}
            ...
            <EditTestSpec>
              <div className="px-4 py-2 bg-white rounded-md shadow dark:bg-zinc-800 text-sm dark:text-zinc-300">
                <span className="font-semibold lowercase">assertly</span> / {data?.name}
              </div>
            </EditTestSpec>
          ...
          <div className="flex w-[33.33%] justify-center">
            <div className="flex-1 custom-drag-region"></div>
            <TabsList>
              <TabsTrigger value="step-builder">Step Builder</TabsTrigger>
              <TabsTrigger value="test-planner">Test Plan</TabsTrigger>
            </TabsList>
            <div className="flex-1 custom-drag-region"></div>
          </div>
          <div className="flex w-[33.33%] gap-2">
        ...
        <div className="flex flex-1 w-full">
          <TabsContent value="step-builder" className="w-full">
            <ResizablePanelGroup direction="horizontal" className="flex px-2">
              <ResizablePanel className="p-[1px]" minSize={30} defaultSize={40}>
                <StepBuilder />
              </ResizablePanel>
              ...
              <ResizablePanel className="p-[1px] ml-2" defaultSize={60}>
                <ResizablePanelGroup direction="vertical" className="flex flex-col">
                  ...
                  <ResizableHandle withHandle />
                  <ResizablePanel defaultSize={40} className="p-[1px] mt-2">
                    <PreviewContext />
                  </ResizablePanel>
                </ResizablePanelGroup>
              </ResizablePanel>
          ...
          <TabsContent value="test-planner">Plan your test here</TabsContent>
        </div>
      </Tabs>
    </div>
  )
