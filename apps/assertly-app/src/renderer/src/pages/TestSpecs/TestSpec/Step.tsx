import { RiCheckboxCircleLine, RiCloseCircleLine, RiDeleteBin6Line, RiDraggable, RiPlayFill } from 'react-icons/ri'
...
const Step = ({ step, updateStep, deleteStep, runStep, isOpen, setIsOpen }: StepProps) => {
  ...
  return (
    ...
    >
      <motion.div
        ...
        layout={false}
      >
        <Collapsible className="w-full" open={isOpen} onOpenChange={(open) => setIsOpen(open)}>
          ...
          <CollapsibleContent className="w-full">
            <div className="w-full flex flex-col border-t dark:border-zinc-700/40 p-4">
              <StepProperties stepType={step.type} stepProperties={step.props} updateProperty={updateProperty} />
            </div>
          </CollapsibleContent>
        </Collapsible>
      </motion.div>
...
const getStepDescription = (stepType: string, stepProperties: Record<string, any>) => {
  switch (stepType) {
    case 'javascript':
...
const StepTitle = ({
  ...
  const [firstRemainingStepId, setFirstRemainingStepId] = useState<string | null>(steps[0]?.id || null)

  useEffect(() => {
    const remainingSteps = steps.filter((step) => !testSpecExecutedStepIds.find((id) => id === step.id))
    setFirstRemainingStepId(remainingSteps[0]?.id || null)
  }, [steps, testSpecExecutedStepIds])
  return (
    <motion.div layout={false} className="flex justify-between items-center w-full transition-none">
      <div className="flex flex-1 h-full px-2 transition-none">
        {isOpen && (
          <div className="py-2">
            <StepTypeSelect stepType={stepType} setStepType={(type: string) => updateStep({ type })} />
          </div>
        )}
        <div className="flex-1 cursor-pointer select-none py-3" onClick={() => setIsOpen(!isOpen)}>
          {!isOpen && (
            <div className="px-2">
              <span className="text-sm font-medium"> {getStepDescription(stepType || '', stepProperties)}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex justify-center items-center px-2 gap-2">
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent align="end" side="right" className="">
                <span className="dark:text-zinc-800 font-medium text-md">{stepReason}</span>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )}

        {stepStatus === 'success' && currentRunningStepId !== stepId && (
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="flex justify-center items-center p-2 text-2xl">
              ...
              <TooltipContent align="end" side="right" className="w-96">
                <span className="dark:text-zinc-800 font-medium text-md">{stepReason}</span>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        ...
        {stepStatus !== 'success' && (
          <Button
            ...
            size={'icon'}
            onClick={runStep}
            disabled={firstRemainingStepId !== stepId || currentRunningStepId === stepId}
            className={cn(
              'text-lg',
              firstRemainingStepId === stepId && 'text-green-500 hover:text-green-400 dark:hover:text-green-400'
            )}
          >
        ...
        <Button variant="outline" size={'icon'} onClick={deleteStep} className="">
          <RiDeleteBin6Line />
        </Button>
        <Button
          variant="outline"
  ...
  const renderProperties = () => {
    switch (stepType) {
      case 'javascript':
        return (
          <>
            <label htmlFor="script" className="block mb-1 text-xs font-semibold text-zinc-700 dark:text-zinc-300">
              JavaScript Code
            </label>
            <textarea
              id="script"
      ...
      case 'ai-assert':
        return (
          <>
            <label htmlFor="assertion" className="block mb-1 text-xs font-semibold text-zinc-700 dark:text-zinc-300">
            ...
            <textarea
              ...
              onChange={(e) => updateProperty('assertion', e.target.value)}
            />
          </>
        )
      case 'goto':
        return (
          <>
            <label htmlFor="url" className="block mb-1 text-xs font-semibold text-zinc-700 dark:text-zinc-300">
              URL
            </label>
            <Input
              id="url"
      ...
      case 'scroll':
        return (
          <>
            <label htmlFor="selector" className="block mb-1 text-xs font-semibold text-zinc-700 dark:text-zinc-300">
              Element to Select
            </label>
            <Input
              id="selector"
              ...
              onChange={(e) => updateProperty('selectorQuery', e.target.value)}
            />
            <label htmlFor="selector" className="block mb-1 text-xs text-zinc-700 dark:text-zinc-300">
            ...
            <div className="flex flex-col mb-2 gap-2">
              ...
              <div className="flex flex-col justify-start items-start">
                <Input
                  ...
                  onChange={(e) => updateProperty('selectorQuery', e.target.value)}
                />
                <label htmlFor="selector" className="block mb-1 text-[10px] text-zinc-400 dark:text-zinc-300 px-1">
                  {stepProperties?.selector || ''}
                </label>
              </div>
            </div>

            <label htmlFor="typeText" className="block mb-1 text-xs font-semibold text-zinc-700 dark:text-zinc-300">
              Text to Type
            </label>
            <Input
              id="typeText"
      ...
      case 'press':
        return (
          <>
            ...
            <label htmlFor="pressKey" className="block mb-1 text-xs font-semibold text-zinc-700 dark:text-zinc-300">
              Key to Press
            </label>
            <Input
              id="pressKey"
        ...
        return (
          <>
            ...
            <Input
              ...
              onChange={(e) => updateProperty('selectorQuery', e.target.value)}
            />
            <label htmlFor="selectValue" className="block mb-1 text-xs font-semibold text-zinc-700 dark:text-zinc-300">
              Value to Select
            </label>
            <Input
              id="selectValue"
      ...
      case 'wait':
        return (
          <>
            ...
            <label htmlFor="waitTimeout" className="block mb-1 text-xs font-semibold text-zinc-700 dark:text-zinc-300">
              Timeout (ms)
            </label>
            <Input
              id="waitTimeout"
