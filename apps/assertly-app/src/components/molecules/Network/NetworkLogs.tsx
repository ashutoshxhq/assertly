            ...
            >
                <CollapsibleTrigger className="flex text-xs py-0 w-full">
                    <div className="flex flex-1 items-center justify-start w-full space-x-2 overflow-hidden">
                        <span className="flex font-bold flex-shrink-0 min-w-[40px]">
                            {log.request.method}
                        </span>
                        ...
                        {log.response && (
                            <span
                                className={`font-bold ${getStatusColor(log.response.status)} flex-shrink-0 ml-2`}
                            >
                                {log.response.status}
                            </span>
                        )}
...
const NetworkLogs = ({ logs }: { logs: NetworkLog[] }) => {
    ...
    useEffect(() => {
        ...
        const placeholders =
            containerRef.current?.querySelectorAll(".log-placeholder");
        ...
        <div ref={containerRef} className="h-full pb-12 overflow-y-auto">
            <div className="flex flex-col gap-1">
                {logs.map((_log, index) => (
                    ...
                    >
                        {visibleLogs[index] && (
                            <NetworkLogItem log={visibleLogs[index]} />
                        )}
                    </div>
                ))}
            </div>
