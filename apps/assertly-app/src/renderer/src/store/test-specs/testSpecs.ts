...
import { selectedProjectIdAtom } from '../projects/projects'
...
export const testSpecsAtom = atomWithQuery((get) => {
  ...
  // Only proceed if we have an projectId
  if (!projectId) {
    return {
      queryKey: ['test-specs', 'loading'],
      queryFn: async () => ({ data: [] })
    }
  }
  ...
  return {
    ...
    queryFn: async ({ queryKey: [, , pagination] }: any) => {
      ...
      const res = await axios.get(`${ENGINE_SERVICE_URL}/teams/${teamId}/test-specs?query=${JSON.stringify(query)}`)
      return res.data
    }
  }
})
...
export const selectedTestSpecAtom = atomWithQuery((get) => {
  ...
  return {
    ...
    queryFn: async () => {
      ...
      const res = await axios.get(`${ENGINE_SERVICE_URL}/teams/${teamId}/test-specs/${selectedTestSpecId}`)
      return res.data
    }
  }
})
...
export const createTestSpecAtom = atomWithMutation((get) => {
  ...
  return {
    ...
    mutationFn: async (data: any) => {
      const res = await axios.post(`${ENGINE_SERVICE_URL}/teams/${teamId}/test-specs`, data)
      return res.data
    },
    onSuccess: () => {
...
export const updateTestSpecMutationAtom = atomWithMutation((get) => {
  ...
  return {
    mutationKey: [teamId, 'test-specs'],
    mutationFn: async (data: any) => {
      const res = await axios.patch(`${ENGINE_SERVICE_URL}/teams/${teamId}/test-specs/${data.where.id}`, data)
      return res.data
    },
    onSuccess: () => {
      get(testSpecsAtom).refetch()
...
export const deleteTestSpecAtom = atomWithMutation((get) => {
  ...
  return {
    ...
    mutationFn: async ({ id, where }: any) => {
      const res = await axios.delete(`${ENGINE_SERVICE_URL}/teams/${teamId}/test-specs/${id}`, { data: { where } })
      return res.data
    },
    onSuccess: () => {
      get(testSpecsAtom).refetch()
