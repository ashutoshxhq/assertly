...
export class WebdriverGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleMessage(client: Socket, message: { actions: Action[] }): Promise<void> {
    try {
      await this.webdriverService.executeActions(client, client.data.id, message.actions)
    } catch (error) {
      client.send(JSON.stringify({ event: 'ERROR', data: { success: false, message: error.message } }));
    }
  }
}
