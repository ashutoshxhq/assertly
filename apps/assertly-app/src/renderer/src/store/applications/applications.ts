...
export const applicationsAtom = atomWithQuery((get) => {
  ...
  return {
    ...
    queryFn: async ({ queryKey: [, , pagination] }: any) => {
      ...
      const res = await axios.get(`${ENGINE_SERVICE_URL}/teams/${teamId}/applications?query=${JSON.stringify(query)}`)
      return res.data
    }
  }
})
...
export const selectedApplicationAtom = atomWithQuery((get) => {
  ...
  return {
    ...
    queryFn: async () => {
      ...
      const res = await axios.get(`${ENGINE_SERVICE_URL}/teams/${teamId}/applications/${selectedApplicationId}`)
      return res.data
    }
  }
})
...
export const createApplicationAtom = atomWithMutation((get) => {
  ...
  return {
    ...
    mutationFn: async (data: any) => {
      const res = await axios.post(`${ENGINE_SERVICE_URL}/teams/${teamId}/applications`, data)
      return res.data
    },
    onSuccess: () => {
...
export const deleteApplicationAtom = atomWithMutation((get) => {
  ...
  return {
    ...
    mutationFn: async ({ id, where }: any) => {
      const res = await axios.delete(`${ENGINE_SERVICE_URL}/teams/${teamId}/applications/${id}`, { data: { where } })
      return res.data
    },
    onSuccess: () => {
      get(applicationsAtom).refetch()
