...
import { authAtom, refreshTokenAtom, refreshTokenMutationAtom } from './store/auth/auth'
...
function PrivateLayout() {
  ...
  useEffect(() => {
    if (refreshToken) {
      ...
      if (!localStorage.getItem('auth_state') || localStorage.getItem('auth_state') === 'null') {
        navigate('/auth/login')
      }
    }
  }, [refreshToken])

  if (!authState || isPending || !authState.isAuthenticated || status === 'pending') {
    return <Loader />
  }
  return (
