...
export class IntegrationsController {
  ...
  async create(
    ...
    const created = await this.integrationsService.create(
  ...
  async findMany(@Query('query') query: string) {
    const matches = await this.integrationsService.findMany({
      crudQuery: query,
    });
    return matches;
  }
  ...
  async findOne(@Param('id') id: string, @Query('query') query: string) {
    const match = await this.integrationsService.findOne(id, {
      crudQuery: query,
    });
    return match;
  }
  ...
  async update(
    ...
    const updated = await this.integrationsService.update(
      id,
