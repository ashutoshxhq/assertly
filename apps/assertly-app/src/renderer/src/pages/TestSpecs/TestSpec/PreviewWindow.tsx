...
const PreviewWindow = () => {
  const [currentTestSpecExecutionScreenshot] = useAtom(currentTestSpecExecutionScreenshotAtom)
  const [currentTestSpecExecutionPageURL] = useAtom(currentTestSpecExecutionPageURLAtom)
  ...
  return (
    <div className="flex flex-1 flex-col h-full grow rounded-lg shadow ring-1 dark:ring-zinc-950/4 ring-zinc-200 bg-white dark:bg-zinc-950 dark:ring-white/5 dark:text-zinc-300 overflow-y-scroll">
      <div className="flex items-center p-2 border-b ">
        ...
        <div className="flex flex-1 gap-1 items-center justify-center">
          <div className="rounded-full flex items-center justify-center bg-zinc-200 dark:bg-zinc-800 px-8 py-1.5 w-80">
            <span className="text-xs dark:text-zinc-300 truncate">{currentTestSpecExecutionPageURL}</span>
          </div>
          <div>
            <Button
              size={'icon'}
              ...
              onClick={() => {
                navigator.clipboard.writeText(currentTestSpecExecutionPageURL)
      ...
      <div className="lex flex-1 bg-zinc-100 dark:bg-zinc-300 w-full h-full bg-cover overflow-y-scroll p-1">
        {currentTestSpecExecutionScreenshot && <StreamedImage imageData={currentTestSpecExecutionScreenshot} />}
      </div>
    </div>
  )
}
...
const StreamedImage: React.FC<{ imageData: string }> = ({ imageData }) => {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      // Swap canvases
      canvasRefs.current[currentCanvasIndex.current]!.style.display = 'none'
  ...
  return (
    <div ref={containerRef} className="w-full relative" style={{ aspectRatio: '16/9' }}>
      {[0, 1].map((index) => (
        <canvas
          key={index}
          ...
          style={{
            display: index === currentCanvasIndex.current ? 'block' : 'none',
            width: '100%',
            height: '100%'
          }}
        />
